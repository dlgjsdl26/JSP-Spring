...
/**
 * Http 의 request 명세
 * 1) request Line : Protocol/version, URL(수신자), Method(요청의 목적, 방법)
 ...
 * 		- OPTION : preflight 요청에 사용, 서버의 메소드 지원여부를 확인할 용도
 * 		- TRACE : 서버를 디버깅할 용도
...
//		CollectionView : iterator, enumeration
		...
		// 요청 파라미터가 어떻게 전달되길래??
		// 파라미터는 특수문자를 '%E3'과 같은 형태로 인코딩이 되어 전달됨. - RFC2396 규약에 따라 정의됨.
		// percent encoding, URL encoding 방식으로 인코딩되도록 정의되어있음.
		String test1 = request.getParameter("test1");
		String test2 = request.getParameter("test2");
		System.out.printf(" test : %s, test2 : %s\n", test1, test2);
	}
